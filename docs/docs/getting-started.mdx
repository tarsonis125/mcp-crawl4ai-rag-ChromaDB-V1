...
Create a new [Supabase project](https://supabase.com/dashboard), then run these SQL scripts **in order** in the SQL Editor:
</Admonition>

1. **`migration/initial_setup.sql`** - Creates vector database, credentials, and core tables
2. **`migration/archon_tasks.sql`** - Creates project and task management tables
...
4. Click **"New Query"**
5. Copy the contents of `migration/initial_setup.sql` from your local files
6. Paste and click **"Run"**
7. Repeat for `migration/archon_tasks.sql`
...
graph TB
    ...
    classDef containerBox fill:#1a1a2e,stroke:#00d4ff,stroke-width:2px,color:#00d4ff
    classDef externalBox fill:#16213e,stroke:#00ff88,stroke-width:2px,color:#00ff88
    classDef coreService fill:#0f3460,stroke:#ff6b6b,stroke-width:2px,color:#ffffff
    
    class Frontend,Backend,MCP,Docs coreService
    class Supabase,OpenAI,Logfire externalBox
```
...
Check the **[MCP Integration Guide](./mcp-reference.mdx)** for specific connection instructions for your AI client.
...
2. **🔌 [Connect Your AI Client](./mcp-reference.mdx)** - Set up Cursor, Windsurf, or Claude Desktop  
...
### 🔌 MCP Integration  
**[MCP Reference](./mcp-reference.mdx)** - Connect Cursor, Windsurf, Claude Desktop, and other MCP clients

...
graph TB
    subgraph "🐳 Docker Microservices Environment"
        subgraph "🚀 API Service (archon-api)"
...
The API service (`archon-api`) is the main entry point for the application:
...
```dockerfile title="Dockerfile.api"
...
# Scale the API service for more concurrent requests
docker compose up -d --scale archon-api=2

...
6. **WebSocket Safety**: Never create real WebSocket connections in tests
...
### 2. Service Testing with WebSocket Safety
...
// CRITICAL: Always mock WebSocket service
vi.mock('@/services/websocketService', () => ({
  websocketService: {
    connect: vi.fn(),
    disconnect: vi.fn(),
...
describe('WebSocket Service (Mocked)', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  })
...
// ✅ CORRECT: Test WebSocket interactions safely
...
// ❌ WRONG: Never create real WebSocket connections
it('should connect to real WebSocket', () => {
  const ws = new WebSocket('ws://localhost:8080') // DON'T DO THIS
...
- [ ] Ensure WebSocket mocking is in place if needed
...
### WebSocket-Specific Checklist
- [ ] WebSocket service is mocked, never real connections
...
// Issue: WebSocket connection errors in tests
...
- 🛠️ [WebSocket Testing](#websocket-testing-best-practices) - Safe WebSocket testing patterns

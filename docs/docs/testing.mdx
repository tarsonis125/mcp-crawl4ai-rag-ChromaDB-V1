...
graph TB
    subgraph "Backend Testing (Python)"
        ...
        PyE2E["E2E Tests<br/>🔍 Full Workflows<br/>MCP Tools, Socket.IO"]
    end
...
- **Streaming Output**: Real-time test results via Socket.IO
...
docker exec -it archon-api pytest --cov=src
```
...
## 🛠️ Socket.IO Testing Strategy

Socket.IO functionality is critical to Archon's real-time features. Our testing approach:

### **Safety First**
- **Never create real Socket.IO connections** in tests
...
- [ ] Socket.IO mocks are properly implemented
...
5. **Socket.IO Safety**: Always mock Socket.IO connections in tests
...
- **[Socket.IO Documentation](./websockets)** - Real-time communication patterns

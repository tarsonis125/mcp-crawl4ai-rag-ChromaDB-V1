...
RAG operations involve CPU-intensive embedding generation, I/O-bound vector searches, and real-time WebSocket updates. Proper threading patterns are essential for performance and stability.
</Admonition>

The RAG system implements sophisticated threading patterns derived from the proven `crawl4ai_mcp.py` architecture, ensuring high performance while maintaining WebSocket connection health.
...
**Frontend WebSocket Safety:**
- ✅ **WebSocket-safe threading utilities** for UI responsiveness
...
```python
# ❌ Blocking operation that kills WebSockets
...
- **🌐 Distributed Processing**: Multi-node RAG processing with WebSocket coordination

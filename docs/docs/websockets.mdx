...
class DatabaseChangeDetector:
    ...
    async def _check_and_broadcast_changes(self, project_id: str):
        ...
        query = """
            SELECT id, title, status, assignee, task_order, updated_at
            FROM tasks 
        ...
        async with get_connection() as conn:
            changed_tasks = await conn.fetch(query, project_id, last_check)
        ...
        if changed_tasks:
            ...
            tasks_data = [dict(task) for task in changed_tasks]
            await self._broadcast_to_project(project_id, {
                "type": "tasks_updated",
                "data": {
                    "project_id": project_id,
                    "updated_tasks": tasks_data,
    ...
    async def _broadcast_to_project(self, project_id: str, message: dict):
        ...
        for ws in disconnected:
            self.remove_websocket_connection(project_id, ws)
...
async def websocket_project_tasks(websocket: WebSocket, project_id: str):
    """WebSocket endpoint for real-time task updates"""
    try:
        await websocket.accept()
        print(f"WebSocket connected for project tasks: {project_id}")
        
        # Add to change detector (starts monitoring automatically)
        await db_change_detector.add_websocket_connection(project_id, websocket)
        
        # Send initial task state
        async with get_connection() as conn:
            tasks = await conn.fetch(
                "SELECT * FROM tasks WHERE project_id = $1 AND archived = false ORDER BY task_order",
                project_id
            )
...
const useRealtimeTaskUpdates = (projectId: string) => {
  ...
  useEffect(() => {
    ...
    ws.onmessage = (event) => {
      ...
      if (message.type === 'initial_tasks') {
        ...
        // Update specific tasks that were changed
        setTasks(currentTasks => {
          const updatedTasks = [...currentTasks];
          message.data.updated_tasks.forEach(updatedTask => {
        ...
        // Show notification
        toast.success(`Tasks updated by AI agent!`);
      }
    };
...
#### 🚨 5. useCallback Dependency Hell (MOST COMMON ISSUE)
**Symptoms**: 

...
<TabItem value="namespace" label="Namespace Class Implementation">

```python
# socketio/crawl_namespace.py
...
<TabItem value="broadcast" label="Progress Management">

```python
# socketio/progress_utils.py
...
<TabItem value="registration" label="Namespace Registration">
...
Archon has fully migrated to Socket.IO. All WebSocket endpoints have been removed and replaced with Socket.IO namespaces.
</Admonition>

### Key Architecture Changes

1. **No More WebSocket Endpoints**: All `/api/crawl-progress/{id}` WebSocket endpoints removed
2. **Namespace-Based Organization**: Progress tracking now uses `/crawl` namespace
3. **Class-Based Handlers**: Socket.IO handlers are organized in namespace classes
4. **Centralized Progress Utils**: Progress functions in `socketio/progress_utils.py`
...
├── socketio/
│   ├── __init__.py
│   ├── crawl_namespace.py      # CrawlNamespace class
│   └── progress_utils.py       # Progress tracking functions
├── socketio_app.py             # Socket.IO server configuration
└── fastapi/
    └── knowledge_api.py        # Registers namespace
```

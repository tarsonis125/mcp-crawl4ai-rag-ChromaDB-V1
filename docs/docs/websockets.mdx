...
**Purpose**: Real-time progress updates during crawling and document processing operations
...
- **📦 Detailed Batch Info**: "Batch 2/9: Processing items 16-30..." with accurate percentages
...
// Initial connection
{
  ...
  "data": {
    ...
    "log": "Analyzing URL type: https://www.example.com",
    "logs": ["Starting crawl...", "Analyzing URL type..."]
  }
}

// Document storage with smooth batch progress
{
  "type": "crawl_progress", 
  "data": {
    "progressId": "8b74d6a0-4970-4d7f-bf82-52a7c08916ad",
...
// Contextual embeddings progress
{
  "type": "crawl_progress",
  "data": {
    "progressId": "8b74d6a0-4970-4d7f-bf82-52a7c08916ad",
    "status": "document_storage",
    "percentage": 38,
    "log": "Batch 3/9: Creating contextual embeddings..."
  }
}
...
// Database storage progress
{
  ...
  "data": {
    ...
    "status": "document_storage",
    "percentage": 42,
    "log": "Batch 3/9: Storing in database..."
  }
}

// Code examples processing (separate phase)
{
  "type": "crawl_progress",
  "data": {
    "progressId": "8b74d6a0-4970-4d7f-bf82-52a7c08916ad",
    "status": "code_storage",
    "percentage": 92,
    "log": "Processed 45/50 code summaries"
  }
}
...
## 🚨 **Common WebSocket Patterns & Anti-Patterns**

### **Duplicate Event Pattern (ANTI-PATTERN)**

**The Problem**: Multiple event sources trigger the same UI action, causing duplicate toasts and unstable state.

```typescript
// ❌ ANTI-PATTERN: Multiple sources showing the same completion toast
// Source 1: Progress callback
const handleProgressComplete = (data: CrawlProgressData) => {
  ...
  loadKnowledgeItems();
  showToast('Crawling completed successfully', 'success'); // ❌ Toast #1
};

// Source 2: WebSocket event handler  
const handleKnowledgeUpdate = (data: any) => {
  if (data.type === 'crawl_completed') {
    setKnowledgeItems(data.data.items);
    showToast('Crawling completed successfully', 'success'); // ❌ Toast #2 (DUPLICATE!)
  }
};

// Source 3: API response (hypothetical)
const completeCrawl = async () => {
  const response = await api.post('/complete-crawl');
  showToast('Crawling completed successfully', 'success'); // ❌ Toast #3 (TRIPLE!)
};
```
...
```typescript
// ✅ PATTERN: Single source of truth for completion events
// Only WebSocket shows completion toast:
const handleKnowledgeUpdate = (data: any) => {
  if (data.type === 'crawl_completed') {
    setKnowledgeItems(data.data.items);
...
// Progress callback does NOT show completion toast:
const handleProgressComplete = (data: CrawlProgressData) => {
  console.log('Crawl completed:', data);
  setProgressItems(prev => prev.filter(item => item.progressId !== data.progressId));
  loadKnowledgeItems();
  // No toast - WebSocket will handle completion notification
};

// API responses only show toasts for direct user actions:
const deleteCrawl = async () => {
  await api.delete('/crawl');
  showToast('Crawl deleted', 'success'); // ✅ Direct user action
};
```

### **useEffect Dependency Hell (ANTI-PATTERN)**

**The Problem**: Including unstable dependencies causes infinite re-renders and WebSocket instability.

```typescript
// ❌ ANTI-PATTERN: Including functions in dependency arrays
...
useEffect(() => {
  const checkHealth = async () => {
    ...
    if (response.ok) {
      setBackendReady(true); // This triggers the effect again!
      showToast('Backend ready', 'success');
    }
  };
  
  checkHealth();
}, [showToast, backendReady]); // ❌ showToast + backendReady cause loops

// ✅ PATTERN: Stable dependencies only
useEffect(() => {
  const checkHealth = async () => {
    const response = await fetch('/health');
    if (response.ok) {
      setBackendReady(true); // Set state, but don't depend on it
      showToast('Backend ready', 'success'); // Call function, but don't depend on it
    }
  };
...
### **WebSocket State Management Best Practices**
...
useEffect(() => {
  ...
  const ws = connectToTaskUpdates(projectId, {
    onTasksChange: (updatedTasks) => {
      setTasks(prev => {
  ...
  return () => {
    ws?.close(); // Always clean up
  };
}, [projectId]); // Only depend on stable values
```

#### **2. Event Deduplication**
```typescript
// ✅ Handle multiple event types without duplication
const wsEventHandlers = {
  initial_tasks: (data) => {
    setTasks(data.tasks);
    // No toast for initial load
  },
  
  tasks_updated: (data) => {
    updateTasksInPlace(data.updated_tasks);
    // No toast - this is automatic from MCP operations
  },
  
  task_created: (data) => {
    addTask(data.task);
    showToast('Task created', 'success'); // ✅ Only for explicit user actions
  }
};
```

#### **3. Database-WebSocket Bridge Pattern**
```typescript
// ✅ Clean separation: Database operations trigger WebSocket updates
// MCP Tool updates database → Database change detector → WebSocket broadcast → UI update

// Backend: Database change detection
async function detectChanges(projectId: string) {
  const changedTasks = await db.tasks
    .where('project_id', projectId)
    .where('updated_at', '>', lastCheckTime)
    .select();
    
  if (changedTasks.length > 0) {
    broadcastToWebSockets(projectId, {
      type: 'tasks_updated',
      data: { updated_tasks: changedTasks }
    });
  }
}

// Frontend: Single handler for all task updates
const handleTaskUpdates = (message) => {
  if (message.type === 'tasks_updated') {
    // Bulk update without showing toast (automatic operation)
    updateTasks(message.data.updated_tasks);
  }
};
```

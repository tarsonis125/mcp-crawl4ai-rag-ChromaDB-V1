...
Archon uses a microservices architecture with three separate services. All services run in Docker containers and communicate via the `app-network` bridge network using service names, not localhost. Real-time features use Socket.IO for reliable WebSocket connections.
</Admonition>
...
| API | archon-api | 8080 | 8080 | FastAPI + Socket.IO |
...
- Frontend → API: `http://archon-api:8080`
...
docker compose build archon-api

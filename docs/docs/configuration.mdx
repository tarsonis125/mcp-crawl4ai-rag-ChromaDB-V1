...
All services run in Docker containers and communicate via the `app-network` bridge network using service names, not localhost. Real-time features use Socket.IO for reliable WebSocket connections.
</Admonition>
...
# Socket.IO Configuration (all defaults work out of the box)
# USE_SOCKETIO_PROGRESS=true  # Enable Socket.IO for real-time features
```

...
```python
# tests/integration/test_websockets/test_task_updates.py
import pytest
import asyncio
from httpx import AsyncClient
...
@pytest.mark.asyncio
class TestTaskUpdatesWebSocket:
    """Test WebSocket endpoints for real-time task updates."""
    
    async def test_websocket_connection_and_initial_tasks(self, client: TestClient, test_project):
        """Test WebSocket connection and initial tasks message."""
    ...
    async def test_websocket_task_updates_from_database_changes(self, client: TestClient, test_project, mock_db_detector):
        """Test that database changes trigger WebSocket updates."""
        with client.websocket_connect(f"/api/projects/{test_project.id}/tasks/ws") as websocket:
            ...
            assert len(data["data"]["updated_tasks"]) == 1
            assert data["data"]["updated_tasks"][0]["status"] == "doing"
    
    async def test_websocket_heartbeat_handling(self, client: TestClient, test_project):
    ...
    async def test_websocket_reconnection_with_invalid_session(self, client: TestClient):
        """Test WebSocket behavior with invalid session ID."""
        # Try to connect with non-existent project
        with pytest.raises(Exception) as exc_info:
            with client.websocket_connect("/api/projects/invalid-id/tasks/ws") as websocket:
                pass
        
        # Should fail to connect
        assert "403" in str(exc_info.value) or "404" in str(exc_info.value)

# tests/integration/test_websockets/test_crawl_progress.py
@pytest.mark.asyncio
class TestCrawlProgressWebSocket:
    """Test WebSocket progress tracking for crawl operations."""
    
    async def test_crawl_progress_streaming(self, client: TestClient, mock_progress_manager):
        """Test real-time progress updates during crawling."""
        progress_id = "test-progress-123"
        
        with client.websocket_connect(f"/api/crawl-progress/{progress_id}") as websocket:
            ...
            assert data["data"]["percentage"] == 25
            assert data["data"]["status"] == "crawling"
            ...
            data = websocket.receive_json()
            assert data["type"] == "crawl_completed"
            assert data["data"]["percentage"] == 100
            assert data["data"]["chunksStored"] == 50
```

...
- **Task Hierarchies**: Creates parent-child task relationships
...
```python
# User request
"Break down 'Implement login API endpoint' into subtasks"

# Task Agent workflow
1. Analyze parent task
...
3. Create subtasks:
   manage_task(action="create", parent_task_id=parent_id, ...)
   
4. Set dependencies:
   - Input validation → Database query
   - Database query → Password verification
   - Password verification → JWT generation

5. Return: "Created 6 subtasks with dependencies"
```
...
### Monitoring Example
```python
with logfire.span("task_agent_operation") as span:
    span.set_attribute("operation_type", operation)
    span.set_attribute("project_id", project_id)
    span.set_attribute("tasks_affected", len(tasks))
    
    result = await execute_operation()
    
    span.set_attribute("success", result.success)
    span.set_attribute("execution_time", result.duration)
```

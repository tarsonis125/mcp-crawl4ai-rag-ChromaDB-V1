...
| Service | Purpose | Key Methods |
|---------|---------|-------------|
| **CrawlOrchestrationService** | Orchestrates entire crawl workflow | `orchestrate_crawl()`, `set_progress_id()` |
...
- Supports multiple providers: OpenAI, OpenRouter, Ollama, Google Gemini
...
- `LLM_PROVIDER` - LLM provider selection (openai, openrouter, ollama, google)
- `CODE_BLOCK_MIN_LENGTH` - Minimum code block length (default: 50)

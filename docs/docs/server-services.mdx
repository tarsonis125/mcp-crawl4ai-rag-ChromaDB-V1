...
| **DocumentStorageService** | Document upload & processing service | `upload_document()`, `store_documents()`, `process_document()` |
...
| Service/Module | Purpose | Key Components |
|---------|---------|-------------|
| **SearchService** | High-level search operations | `perform_rag_query()`, `search_code_examples_service()`, `rerank_results()` |
| **vector_search_service.py** | Vector search utilities | `search_documents()`, `search_code_examples()` |
...
- **Utility Modules**: Low-level functions for specific operations (e.g., `vector_search_service.py`, `document_storage_service.py`)
- **Service Classes**: High-level orchestration of utilities (e.g., `SearchService`, `DocumentStorageService`)
- **Base Classes**: Abstract implementations for extensibility (e.g., `BaseStorageService`)

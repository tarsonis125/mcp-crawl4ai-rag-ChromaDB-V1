...
### Monitoring Example
```python
with logfire.span("rag_agent_search") as span:
    span.set_attribute("original_query", query)
    span.set_attribute("enhanced_query", enhanced_query)
    span.set_attribute("sources_searched", len(sources))
    
    results = await perform_search(enhanced_query)

...
|-----------------|----------|---------|
| **RAG Services** | `SourceManagementService`<br/>`CrawlingService`<br/>`SearchService`<br/>`DocumentStorageService` | Knowledge base operations |
...
3. FastAPI endpoint uses SourceManagementService.delete_source()
...
3. Server endpoint uses SourceManagementService.delete_source()
4. Service deletes from database
...
3. FastAPI endpoint:
   ...
   - Uses DocumentStorageService for chunking
...
```
MCP Tools → HTTP API → Services → Socket.IO Rooms → UI
```

Key improvements:
- **No database polling** - Eliminated 2-second polling system
- **Direct emission** - Services emit Socket.IO events directly
- **Room-based targeting** - Updates go to specific project/progress rooms
- **No callback chains** - Simplified progress tracking
- **Single communication method** - Only Socket.IO, no dual WebSocket/Socket.IO
...
- **Direct Socket.IO emission** - Services emit to rooms directly
- **Socket.IO only** - No WebSocket endpoints, simplified real-time communication
...
<Admonition type="warning" icon="⚠️" title="Critical: MCP is a Lightweight HTTP-Based Protocol Wrapper">

**The MCP service is NOT a business logic layer. It is simply a protocol adapter that:**
- Makes HTTP calls to the Server service for all operations
- Wraps Server HTTP endpoints as MCP tools
- Handles MCP protocol communication with AI clients
- Contains NO business logic, ML models, or data operations
- Maintains minimal dependencies for true microservices separation

**ALL actual functionality comes from the Server service via HTTP APIs.**
...
graph TB
    ...
    subgraph "🔥 Archon MCP Service (Port 8051)"
    ...
    subgraph "📦 Microservices"
        API["⚡ Server Service<br/>Archon-Server:8080"]
        Agents["🤖 Agents Service<br/>archon-agents:8052"]
    end
    
    subgraph "🏗️ Backend Services"
        RAGModule["🧠 RAG Module<br/>7 tools"]
        ProjectModule["📊 Project Module<br/>7 tools"]
...
Archon provides 14 comprehensive MCP tools organized into three main categories:

### 🧠 RAG & Knowledge Management Tools (7 tools)
...
### 📊 Project & Task Management Tools (5 tools with action-based patterns)

| Tool | Description | Actions Available |
|------|-------------|-------------------|
| `manage_project` | Unified project management | `create`, `list`, `get`, `delete` |
| `manage_task` | Complete task lifecycle | `create`, `list`, `get`, `update`, `delete`, `archive` |
| `manage_document` | Document management | `add`, `list`, `get`, `update`, `delete` |
| `manage_versions` | Version control for documents | `create`, `list`, `get`, `restore` |
| `get_project_features` | Retrieve project features | Direct query (no actions) |

### 🏥 System & Monitoring Tools (2 tools)

| Tool | Description | Use Case |
|------|-------------|----------|
| `health_check` | System health diagnostics | Monitor server status, database connectivity |
| `session_info` | MCP session management | View active sessions and connection info |

## 🎯 How MCP Tools Work

<Admonition type="tip" icon="🔧" title="Lightweight HTTP-Based Architecture">

**The MCP service is designed as a lightweight protocol wrapper that:**
1. **Receives MCP tool calls** from AI clients (Cursor, Windsurf, Claude)
2. **Translates them to HTTP requests** to the Server service (port 8080)
3. **Returns formatted responses** back to the AI client

**Example flow for a RAG query:**
```
AI Client → MCP Tool Call → MCP Service → HTTP POST to Server:8080/api/rag/query → Response → AI Client
```

This architecture ensures:
- ✅ **True microservices separation** - Services communicate only via HTTP
- ✅ **Scalability** - Each service can be scaled independently
- ✅ **Maintainability** - Clear boundaries between services
- ✅ **Flexibility** - Easy to add new tools without modifying core logic

</Admonition>

## 🏗️ Microservices Integration

<Admonition type="info" icon="🔧" title="Service Communication">
...
# Open http://localhost:3737 → Settings → Add OpenAI API key

...
<Admonition type="warning" icon="⚠️" title="Critical: MCP is a Thin Protocol Wrapper">

**The MCP service is NOT a business logic layer. It is simply a protocol adapter that:**
- Imports services directly from the Server (`from src.server.services import ...`)
- Wraps Server functions as MCP tools
- Handles MCP protocol communication with AI clients
- Contains NO business logic, ML models, or data operations

**ALL actual functionality comes from the Server service.**

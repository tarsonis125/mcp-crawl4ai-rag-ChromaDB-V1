...
### Logfire Traces
```python
with logfire.span("document_agent_process") as span:
    ...
    span.set_attribute("chunks_created", result.chunk_count)
    span.set_attribute("processing_time", result.duration)
```

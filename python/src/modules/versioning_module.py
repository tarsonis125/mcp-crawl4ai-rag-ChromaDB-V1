...
def register_versioning_tools(mcp: FastMCP):
    ...
    async def create_task_version(
        ...
        """
        Create a version snapshot for a task JSONB field.
        
        Args:
            ctx: MCP server context
            task_id: UUID of the task
            field_name: Name of the JSONB field ('sources', 'code_examples')
            content: The current content to snapshot
            change_summary: Human-readable description of changes
            change_type: Type of change ('create', 'update', 'delete', 'restore')
            created_by: Who created this version
        ...
        try:
            ...
            existing_versions = supabase_client.table("document_versions").select("version_number").eq("task_id", task_id).eq("field_name", field_name).order("version_number", desc=True).limit(1).execute()
            ...
            return json.dumps({
                "success": False,
                "error": f"Error creating task version: {str(e)}"
            })
    ...
    async def get_task_version_history(
        ...
        try:
            ...
            # Build query
            query = supabase_client.table("document_versions").select("*").eq("task_id", task_id)
            ...
            return json.dumps({
                "success": False,
                "error": f"Error getting task version history: {str(e)}"
            })
    ...
    async def restore_task_version(
        ...
        try:
            ...
            # Get the version to restore
            version_result = supabase_client.table("document_versions").select("*").eq("task_id", task_id).eq("field_name", field_name).eq("version_number", version_number).execute()
            
            if not version_result.data:
                return json.dumps({
                    "success": False,
                    "error": f"Version {version_number} not found for {field_name} in task {task_id}"
                })
            ...
            # Get current content to create backup
            current_task = supabase_client.table("tasks").select(field_name).eq("id", task_id).execute()
            if current_task.data:
                current_content = current_task.data[0].get(field_name, {})
            ...
            # Restore the content to task
            update_data = {
                field_name: content_to_restore,
                'updated_at': datetime.now().isoformat()
            }
            
            restore_result = supabase_client.table("tasks").update(update_data).eq("id", task_id).execute()
            ...
            return json.dumps({
                "success": False,
                "error": f"Error restoring task version: {str(e)}"
            })
    ...
    async def get_version_content(
        ...
        Args:
            ...
            task_id: UUID of the task (for task versions)
        ...
        try:
            ...
            # Build query based on whether it's project or task version
            if project_id:
                query = supabase_client.table("document_versions").select("*").eq("project_id", project_id)
            elif task_id:
                query = supabase_client.table("document_versions").select("*").eq("task_id", task_id)
                ...
                return json.dumps({
                    "success": False,
                    "error": "Either project_id or task_id must be provided"
                })
...
async def get_task_version_history_direct(task_id: str, field_name: str = None) -> str:
    """
    Direct function for getting task version history that can be called from FastAPI.
    """
    try:
        supabase_client = get_supabase_client()
        ...
        if result.data is not None:
            ...
            return json.dumps({
                "success": False,
                "error": "Failed to retrieve task version history"
            })
...
async def get_version_content_direct(project_id: str = None, task_id: str = None, field_name: str = None, version_number: int = None) -> str:
    """
    ...
    try:
        ...
        # Build query
        query = supabase_client.table("document_versions").select("*").eq("field_name", field_name).eq("version_number", version_number)
...
async def restore_task_version_direct(task_id: str, field_name: str, version_number: int, restored_by: str = "system") -> str:
    """
    Direct function for restoring task version that can be called from FastAPI.
    """
    try:
        supabase_client = get_supabase_client()
        
        # Get the version to restore
        version_result = supabase_client.table("document_versions").select("*").eq("task_id", task_id).eq("field_name", field_name).eq("version_number", version_number).single().execute()
        ...
        # Update the task with the restored content
        update_data = {
            field_name: version_content,
            "updated_at": datetime.now().isoformat()
        }
        ...
        if restore_result.data:
            ...
            return json.dumps({
                "success": False,
                "error": "Failed to restore task data"
            })
...
async def create_document_version_direct(
    ...
    """
    Direct function for creating document versions that can be called from FastAPI.

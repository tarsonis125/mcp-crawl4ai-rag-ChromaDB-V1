...
export const MCPPage = () => {
  ...
  const handleTestTool = async () => {
    ...
    try {
      // Call the MCP tool directly via mcpService
      const result = await mcpService.callTool(selectedTool.name, testParams);
      setTestResult(result);
      showToast(`Tool "${selectedTool.name}" executed successfully`, 'success');
    } catch (error: any) {
      console.error('MCP tool test error:', error);
      setTestError(error.message || 'Failed to execute MCP tool');
      showToast(`MCP tool test failed: ${error.message}`, 'error');
    } finally {

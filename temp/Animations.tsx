...
export const ArchonLoadingSpinner: React.FC<{
  ...
  size = 'md',
  logoSrc = 'https://uploadthingy.s3.us-west-1.amazonaws.com/gGU2rbCQtMFZ1KPHVcWLbu/logo-neon.svg',
  className = '',
}) => {
  ...
  return (
    <div
      className={`relative ${sizeMap[size].container} flex items-center justify-center ${className}`}
    >
      {/* Central logo */}
      <img
        src={logoSrc}
        alt="Loading"
        className={`${sizeMap[size].logo} z-10 relative`}
      />
...
export const NeonGlowEffect: React.FC<{
  ...
  className?: string
}> = ({ children, color = 'blue', intensity = 'medium', className = '' }) => {
  // Color mappings for different neon colors
  const colorMap = {
    cyan: 'border-cyan-400 shadow-cyan-400/50 dark:shadow-cyan-400/70',
    fuchsia:
      'border-fuchsia-400 shadow-fuchsia-400/50 dark:shadow-fuchsia-400/70',
    ...
    green:
      'border-emerald-500 shadow-emerald-500/50 dark:shadow-emerald-500/70',
  ...
  return (
    <div className={`relative ${className}`}>
      <div
        className={`absolute inset-0 rounded-md border ${colorMap[color]} ${intensityMap[intensity]}`}
      ></div>
...
export const EdgeLitEffect: React.FC<{
  ...
  const colorMap = {
    ...
    green:
      'bg-emerald-500 shadow-[0_0_10px_2px_rgba(16,185,129,0.4)] dark:shadow-[0_0_20px_5px_rgba(16,185,129,0.7)]',
    pink: 'bg-pink-500 shadow-[0_0_10px_2px_rgba(236,72,153,0.4)] dark:shadow-[0_0_20px_5px_rgba(236,72,153,0.7)]',
  }
  return (
    <div
      className={`absolute top-0 left-0 w-full h-[2px] ${colorMap[color]} ${className}`}
    ></div>
  )
}
